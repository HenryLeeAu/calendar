...
const calendarArray = (
  currentYear: number,
  currentMonth: number,
): Array<Array<any | {|date: number, month: number, year: number|}>> => {
  ...
  const getFirstArrraycurrentDays = () => 7 - getFirstDayIndexOfCurrentMonth();
  ...
  const firstArray = () => {
    ...
    for (let i = 0; i < getFirstArrraycurrentDays(); i++) {
  ...
  const middleArrays = () => {
    const array = [];
    const getWeeks = () => (
      Math.floor((getDaysOfCurrentMonth() - getFirstArrraycurrentDays()) / 7)
    );
  ...
  const lastArray = () => {
    const arr = [];
    const getLastArrayNumber = () => (getDaysOfCurrentMonth() - getFirstArrraycurrentDays()) % 7;

    if (getLastArrayNumber() > 0) {
      // excute only has days left
      for (
